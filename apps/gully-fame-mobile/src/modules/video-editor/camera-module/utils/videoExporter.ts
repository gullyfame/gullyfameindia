...
export async function exportAndCombineClips(
  ...
  try {
    ...
    console.log("Cache directory already exists or error creating it");
  }
  ...
  try {
    ...
    if (!ReturnCode.isSuccess(returnCode)) {
      let failureStackTrace = "Unknown error";
      try {
        failureStackTrace = (await session.getFailStackTrace()) || "Unknown error";
      } catch (stackError) {
        console.warn("Could not get failure stack trace:", stackError);
      }
    ...
    throw new Error(`Export failed: ${error.message || "Unknown FFmpeg error"}`);
  }
}
